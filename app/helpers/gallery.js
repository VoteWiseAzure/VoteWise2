...
module.exports.UploadFile = function (data, file_name, res, app) {
  // Making new adress
  ...
  gallery.save( function ( err, gallery ) {
    if (err) {
      //res.status(400);
      ////console.log("err: ",err);
      return res.json( { success: false, error: "Unable to add File" } );
    }
...
module.exports.getList = function (params, res, app) {  
  var query = ''
   console.log(params);
   if(params) {
    
    
    Gallery.find({ "createdBy.id": params })
    .sort({"createdOn": -1})
    .exec(function(err,restop) {
      ...
      if (err) {
        //res.status(400);
        ////console.log("err: ",err);
        return res.json( { success: false, error: "There are no items" } );
      }
